...
A fault left running is worse than one that never ran, so revert must survive
the script dying, the terminal closing, or a laptop going to sleep. Rather than
...
So the objects are `mj-NN`, and the workloads they create are named for what
they plausibly are (`media-transcoder`, `load-driver`). The mapping from id to
...
**Required whenever `clusterScoped=false`**, via
`chaos-mesh-namespaced-rbac-fix.yaml`.
...
This fails in the worst possible way: the API server accepts the CR, `kubectl
...
not widen what Chaos Mesh can act on -- injection still comes from the
namespaced RoleBinding the chart creates for the target namespace. No restart is
needed; client-go retries the watch and the cache syncs within about a minute.
